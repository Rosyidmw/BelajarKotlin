...
fun manipulateString(str: String, int: Int): String {
    val angkaPertama = str.indexOfFirst { it.isDigit() }

    if (angkaPertama != -1) {
        val angka = str.substring(angkaPertama).toInt()
        val banyakAngka = angka * int

        return str.substring(0, angkaPertama) + banyakAngka
    } else {
        return str + int
    }
}
