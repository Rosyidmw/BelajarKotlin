...
fun minAndMax(number: Int): Int {
    var angkaTerbesar = 0
    var angkaTerkecil = 9
    val angkaString = number.toString()

    for (angka in angkaString) {
        val digit = angka.toString().toInt()
        angkaTerkecil = minOf(angkaTerkecil, digit)
        angkaTerbesar = maxOf(angkaTerbesar, digit)
    }

    return angkaTerkecil + angkaTerbesar
}
